...
import { IResult, AppNotificationKey, IContestEntry } from "src/app/interfaces";
  ...
  reject = createEffect(() =>
    this.action$.pipe(
      ...
      concatMap((action: PendingEntryAction.RejectEntry) =>
        this.adminApprovalService.rejectEntry(action.payload.entryId).pipe(
          map(
            (resp: IResult<boolean>) =>
